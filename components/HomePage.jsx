...
// ─────────────────────────────────────────────────────────────────────────────
// Airport database — includes country/region aliases for natural language input
// e.g. "india" → DEL, "uk" → LHR, "america" → JFK
// ─────────────────────────────────────────────────────────────────────────────
const AIRPORTS = [
  // North America
  ...
  // India — comprehensive list so "india" → DEL
  // "india" as a country alias → DEL (primary hub)
...
// ─────────────────────────────────────────────────────────────────────────────
// findAirport — strict token-based matching to prevent false positives
// e.g. "india" must match whole-word, not as a substring of "indiana"
// ─────────────────────────────────────────────────────────────────────────────
function findAirport(text) {
  if (!text) return null;
  const lower = text.trim().toLowerCase();

  // 1. Exact code match (e.g. "DEL", "LHR")
  const byCode = AIRPORTS.find(a => a.code.toLowerCase() === lower);
  if (byCode) return byCode;

  // 2. Whole-word alias match — split input into tokens and compare
  const tokens = lower.split(/[\s,\-\/]+/);

  // Try longest alias match first (greedy: "new york city" before "york")
  ...
  for (const airport of sorted) {
    for (const alias of airport.aliases) {
      ...
      // Check if all alias tokens appear in the input tokens
      if (aliasTokens.every(at => tokens.includes(at))) return airport;
      // Also check if the input contains the alias as a whole substring
...
// ─────────────────────────────────────────────────────────────────────────────
// parseQuery — extract FROM / TO from natural language
// Handles patterns like:
//   "from india to london"
//   "book a flight from india to london"
//   "india to london"
//   "london from india"
// ─────────────────────────────────────────────────────────────────────────────
function parseQuery(query) {
  ...
  // Capture everything between "from" and "to", and after "to"
...
export default function HomePage() {
  ...
  const handleQueryChange = (e) => {
    ...
    if (val.trim().length > 3) {
      ...
      // Show hint if we partially parsed
  ...
  return (
    ...
    >
      ...
      <aside style={{
        ...
        zIndex: 10, flexShrink: 0, paddingTop: 10, paddingBottom: 12, gap: 0,
      }}>
        <div style={{ display:"flex",alignItems:"center",justifyContent:"center",width:28,height:24,cursor:"pointer",marginBottom:10 }}>
        ...
        <div style={{ width:26,height:26,borderRadius:"50%",background:"rgba(0,0,0,0.08)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:18,cursor:"pointer",flexShrink:0 }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:4,width:"100%" }}>
          {NAV_ITEMS.map((item, i) => (
            <div key={i} style={{ display:"flex",alignItems:"center",justifyContent:"center",width:32,height:32,borderRadius:10,cursor:"pointer",color:item.active?"#1a3a35":"rgba(0,0,0,0.22)",background:item.active?"rgba(26,58,53,0.09)":"transparent",transition:"background 0.15s" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d={item.path} /></svg>
            </div>
          ))}
        </div>
        <div style={{ flex:1 }} />
        <div style={{ display:"flex",alignItems:"center",justifyContent:"center",width:32,height:32,borderRadius:10,cursor:"pointer",color:"rgba(0,0,0,0.22)",marginBottom:4 }}>
        ...
        <div style={{ display:"flex",alignItems:"center",justifyContent:"center",width:32,height:32,borderRadius:10,cursor:"pointer",color:"rgba(0,0,0,0.22)",marginBottom:10 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
            <path d="M10.09 15.59L11.5 17l5-5-5-5-1.41 1.41L12.67 11H3v2h9.67l-2.58 2.59zM19 3H5c-1.11 0-2 .9-2 2v4h2V5h14v14H5v-4H3v4c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2z" />
          </svg>
        </div>
        <div style={{ width:26,height:26,borderRadius:"50%",background:"#f5c518",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,boxShadow:"0 2px 6px rgba(245,197,24,0.4)" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
        ...
        {/* TOP BAR */}
        <header style={{ display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,position:"relative",padding:"0 28px",height:46,background:"rgba(238,242,236,0.88)",backdropFilter:"blur(12px)",borderBottom:"1px solid rgba(26,58,53,0.08)" }}>
          <div style={{ display:"flex",alignItems:"center",gap:7 }}>
            <div style={{ width:6,height:6,borderRadius:"50%",background:"#c8f135",flexShrink:0 }} />
            <span style={{ fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:700,letterSpacing:"0.22em",color:"#1a3a35" }}>KAIVO</span>
          </div>
          <div style={{ position:"absolute",left:"50%",transform:"translateX(-50%)",display:"flex",alignItems:"center",gap:4,fontSize:10,fontWeight:500,color:"rgba(14,28,25,0.38)",letterSpacing:"0.06em",whiteSpace:"nowrap" }} className="hidden-mobile">
          ...
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            <span style={{ fontSize:10,fontWeight:700,letterSpacing:"0.12em",color:"rgba(14,28,25,0.32)" }} className="hidden-mobile">HUMID</span>
        ...
        {/* ── HERO / MAIN AREA ── */}
        <main style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",padding:"24px 16px 20px",background:"linear-gradient(160deg,#eef2ec 0%,#e8eeea 35%,#eaf0f5 65%,#ede9f5 100%)" }}>
          ...
          {/* HEADLINE */}
          <div style={{ textAlign:"center",marginBottom:"clamp(18px,3vw,32px)",position:"relative",zIndex:1 }}>
            <h1 style={{ margin:0,lineHeight:1.15,fontFamily:"Georgia,'Times New Roman',serif",fontSize:"clamp(28px,4.5vw,52px)",fontWeight:700,color:"#0e1c19",letterSpacing:"-0.02em" }}>
              Your next flight,
            </h1>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,flexWrap:"wrap",lineHeight:1.15 }}>
              <span style={{ display:"inline-flex",alignItems:"center",fontFamily:"Georgia,'Times New Roman',serif",fontSize:"clamp(28px,4.5vw,52px)",fontWeight:700,color:"#053C43",background:"#c8f135",borderRadius:50,paddingLeft:"0.5em",paddingRight:"0.5em",paddingTop:"0.05em",paddingBottom:"0.1em",letterSpacing:"-0.02em",lineHeight:1.2 }}>
                one ask
              </span>
              <span style={{ fontFamily:"Georgia,'Times New Roman',serif",fontSize:"clamp(28px,4.5vw,52px)",fontWeight:700,color:"#0e1c19",letterSpacing:"-0.02em",lineHeight:1.2 }}>
                away.
              </span>
            </div>
            <p style={{ marginTop:8,fontSize:"clamp(12px,1.5vw,14px)",fontWeight:400,color:"rgba(14,28,25,0.42)" }}>
          ...
          <div style={{ width:"100%",maxWidth:700,position:"relative",zIndex:1,borderRadius:18,padding:10,background:"#ffffff",border:"1px solid rgba(255,255,255,0.9)",boxShadow:"0 2px 8px rgba(14,28,25,0.06),0 12px 32px rgba(14,28,25,0.12),0 32px 80px rgba(14,28,25,0.14)" }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:10,borderRadius:12,transition:"all 0.2s",padding:"10px 12px",marginBottom:2,background:inputFocused?"#ffffff":"#fafbf9",border:`1.5px solid ${inputFocused?"#1a3a35":"rgba(26,58,53,0.12)"}`,boxShadow:"0 0 0 1px rgba(181,250,230,0.8),0 0 20px rgba(180,255,235,0.35),0 8px 24px rgba(180,255,235,0.15),inset 0 1px 0 rgba(255,255,255,0.9)" }}>
              ...
              <input
                ...
                style={{ flex:1,border:"none",outline:"none",background:"transparent",fontFamily:"'DM Sans',sans-serif",fontSize:"clamp(12px,1.5vw,13.5px)",color:"#0e1c19",minWidth:0 }}
              />
            ...
            <div style={{ padding:"4px 6px 6px" }}>
              ...
              {/* Airport + date row */}
              <div style={{ display:"flex",alignItems:"center",flexWrap:"wrap",gap:0,borderTop:"1px solid rgba(26,58,53,0.08)",borderBottom:"1px solid rgba(26,58,53,0.08)",padding:"14px 0",marginBottom:10 }}>

                {/* FROM */}
                <div style={{ display:"flex",flexDirection:"column",gap:2,flex:"1 1 80px",minWidth:70 }}>
                  <span style={{ fontSize:9,fontWeight:700,letterSpacing:"0.14em",color:"rgba(14,28,25,0.38)" }}>FROM</span>
                  <span style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"clamp(20px,3.5vw,28px)",fontWeight:800,lineHeight:1,color:"#1a3a35",letterSpacing:"-0.01em" }}>{from.code}</span>
                  <span style={{ fontSize:9.5,fontWeight:400,color:"rgba(14,28,25,0.36)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:110,marginTop:2 }}>{from.name}</span>
                </div>
                ...
                <div style={{ display:"flex",flexDirection:"column",gap:2,flex:"1 1 80px",minWidth:70 }}>
                  ...
                  <span style={{ fontFamily:"'DM Sans',sans-serif",fontSize:"clamp(20px,3.5vw,28px)",fontWeight:800,lineHeight:1,color:"#1a3a35",letterSpacing:"-0.01em" }}>{to.code}</span>
                  <span style={{ fontSize:9.5,fontWeight:400,color:"rgba(14,28,25,0.36)",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:110,marginTop:2 }}>{to.name}</span>
                </div>

                {/* DEPART */}
                <div style={{ display:"flex",flexDirection:"column",gap:2,flex:"1 1 80px",minWidth:70,paddingLeft:10,borderLeft:"1px solid rgba(26,58,53,0.08)" }}>
                ...
                {/* WHEN */}
                <div style={{ display:"flex",flexDirection:"column",gap:2,flex:"1 1 60px",minWidth:55,paddingLeft:10,borderLeft:"1px solid rgba(26,58,53,0.08)" }}>
                ...
                {/* Ask button */}
                <button onClick={handleFormAsk} style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:7,border:"none",cursor:"pointer",borderRadius:12,transition:"all 0.15s",whiteSpace:"nowrap",marginLeft:8,padding:"0 18px",height:48,flexShrink:0,fontFamily:"'DM Sans',sans-serif",fontSize:"clamp(13px,1.5vw,14px)",fontWeight:700,background:"linear-gradient(135deg,#c8f135 0%,#d4f54a 100%)",color:"#0e1c19",boxShadow:"0 4px 18px rgba(200,241,53,0.38),0 1px 4px rgba(200,241,53,0.2)" }}>
                  Ask
